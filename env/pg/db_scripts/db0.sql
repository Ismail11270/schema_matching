...
CREATE TABLE db.db (
                       ...
                       CONSTRAINT "CK_db_dbType" CHECK (((db_type IS NULL) OR (upper((db_type)::text) = ANY (ARRAY['SC'::text, 'VC'::text, 'IN'::text, 'EM'::text, 'SP'::text, 'GC'::text]))))
);
...
CREATE TABLE db.vendor (
                           ...
                           db_web_service_url character varying(1024),
                           modified_date timestamp without time zone DEFAULT now() NOT NULL,
...
CREATE TABLE db.db_credit_card (
                                   business_entity_id integer NOT NULL,
                                   credit_card_id integer NOT NULL,
                                   modified_date timestamp without time zone DEFAULT now() NOT NULL
);

ALTER TABLE db.db_credit_card OWNER TO postgres;
...
CREATE TABLE db.db_order_detail (
                                    ...
                                    row_guid uuid NOT NULL,
                                    modified_date timestamp without time zone DEFAULT now() NOT NULL,
                                    CONSTRAINT "CK_dbOrderDetail_OrderQty" CHECK ((order_qty > 0)),
...
CREATE TABLE db.db_order_header (
                                    db_order_id integer NOT NULL,
                                    revision_number smallint DEFAULT 0 NOT NULL,
                                    order_date timestamp without time zone DEFAULT now() NOT NULL,
                                    due_date timestamp without time zone NOT NULL,
                                    ...
                                    row_guid uuid NOT NULL,
                                    modified_date timestamp without time zone DEFAULT now() NOT NULL,
                                    CONSTRAINT "CK_dbOrderHeader_DueDate" CHECK ((due_date >= order_date)),
                                    CONSTRAINT "CK_dbOrderHeader_Freight" CHECK ((freight >= 0.00)),
                                    CONSTRAINT "CK_dbOrderHeader_ShipDate" CHECK (((ship_date >= order_date) OR (ship_date IS NULL))),
                                    CONSTRAINT "CK_dbOrderHeader_Status" CHECK (((status >= 0) AND (status <= 8))),
                                    CONSTRAINT "CK_dbOrderHeader_SubTotal" CHECK ((subtotal >= 0.00)),
                                    CONSTRAINT "CK_dbOrderHeader_TaxAmt" CHECK ((tax_amt >= 0.00))
);

ALTER TABLE db.db_order_header OWNER TO postgres;

CREATE TABLE db.db_order_header_db_reason (
                                              db_order_id integer NOT NULL,
                                              db_reason_id integer NOT NULL,
                                              modified_date timestamp without time zone DEFAULT now() NOT NULL
);

ALTER TABLE db.db_order_header_db_reason OWNER TO postgres;
...
CREATE TABLE db.db_db (
                          ...
                          CONSTRAINT "CK_dbdb_CommissionPct" CHECK ((commission_pct >= 0.00)),
                          CONSTRAINT "CK_dbdb_dbLastYear" CHECK ((db_last_year >= 0.00)),
                          CONSTRAINT "CK_dbdb_dbQuota" CHECK ((db_quota > 0.00)),
                          CONSTRAINT "CK_dbdb_dbYTD" CHECK ((db_ytd >= 0.00))
);

ALTER TABLE db.db_db OWNER TO postgres;

CREATE TABLE db.db_db_quota_history (
                                        business_entity_id integer NOT NULL,
                                        quota_date timestamp without time zone NOT NULL,
                                        db_quota numeric NOT NULL,
                                        row_guid uuid NOT NULL,
                                        modified_date timestamp without time zone DEFAULT now() NOT NULL,
                                        CONSTRAINT "CK_dbdbQuotaHistory_dbQuota" CHECK ((db_quota > 0.00))
);

ALTER TABLE db.db_db_quota_history OWNER TO postgres;
...
CREATE TABLE db.db_territory_history (
                                         business_entity_id integer NOT NULL,
                                         territory_id integer NOT NULL,
                                         start_date timestamp without time zone NOT NULL,
                                         end_date timestamp without time zone,
                                         row_guid uuid NOT NULL,
                                         modified_date timestamp without time zone DEFAULT now() NOT NULL,
                                         CONSTRAINT "CK_dbTerritoryHistory_EndDate" CHECK (((end_date >= start_date) OR (end_date IS NULL)))
);

ALTER TABLE db.db_territory_history OWNER TO postgres;
...
CREATE SEQUENCE db.dborderheader_dborderid_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
ALTER TABLE db.dborderheader_dborderid_seq OWNER TO postgres;

ALTER SEQUENCE db.dborderheader_dborderid_seq OWNED BY db.db_order_header.db_order_id;
...
ALTER TABLE ONLY db.db_order_header ALTER COLUMN db_order_id SET DEFAULT nextval('db.dborderheader_dborderid_seq'::regclass);
...
ALTER TABLE ONLY db.business_entity_contact
    ADD CONSTRAINT "PK_BusinessEntityContact_BusinessEntityID_dbID_ContactTypeI" PRIMARY KEY (business_entity_id, db_id, contact_type_id);
...
ALTER TABLE ONLY db.db_credit_card
    ADD CONSTRAINT "PK_dbCreditCard_BusinessEntityID_CreditCardID" PRIMARY KEY (business_entity_id, credit_card_id);

ALTER TABLE db.db_credit_card CLUSTER ON "PK_dbCreditCard_BusinessEntityID_CreditCardID";
...
ALTER TABLE ONLY db.db_order_header_db_reason
    ADD CONSTRAINT "PK_dbOrderHeaderdbReason_dbOrderID_dbReasonID" PRIMARY KEY (db_order_id, db_reason_id);

ALTER TABLE db.db_order_header_db_reason CLUSTER ON "PK_dbOrderHeaderdbReason_dbOrderID_dbReasonID";

ALTER TABLE ONLY db.db_order_header
    ADD CONSTRAINT "PK_dbOrderHeader_dbOrderID" PRIMARY KEY (db_order_id);

ALTER TABLE db.db_order_header CLUSTER ON "PK_dbOrderHeader_dbOrderID";

ALTER TABLE ONLY db.db_db_quota_history
    ADD CONSTRAINT "PK_dbdbQuotaHistory_BusinessEntityID_QuotaDate" PRIMARY KEY (business_entity_id, quota_date);

ALTER TABLE db.db_db_quota_history CLUSTER ON "PK_dbdbQuotaHistory_BusinessEntityID_QuotaDate";
...
ALTER TABLE ONLY db.db_territory_history
    ADD CONSTRAINT "PK_dbTerritoryHistory_BusinessEntityID_StartDate_TerritoryID" PRIMARY KEY (business_entity_id, start_date, territory_id);

ALTER TABLE db.db_territory_history CLUSTER ON "PK_dbTerritoryHistory_BusinessEntityID_StartDate_TerritoryID";
...
ALTER TABLE ONLY db.customer
    ADD CONSTRAINT "FK_Customer_dbTerritory_TerritoryID" FOREIGN KEY (territory_id) REFERENCES db.db_territory(territory_id);

ALTER TABLE ONLY db.customer
    ADD CONSTRAINT "FK_Customer_Store_StoreID" FOREIGN KEY (store_id) REFERENCES db.store(business_entity_id);

ALTER TABLE ONLY db.db_credit_card
    ADD CONSTRAINT "FK_dbCreditCard_CreditCard_CreditCardID" FOREIGN KEY (credit_card_id) REFERENCES db.credit_card(credit_card_id);

ALTER TABLE ONLY db.db_credit_card
    ADD CONSTRAINT "FK_dbCreditCard_db_BusinessEntityID" FOREIGN KEY (business_entity_id) REFERENCES db.db(business_entity_id);

ALTER TABLE ONLY db.db_order_detail
    ADD CONSTRAINT "FK_dbOrderDetail_dbOrderHeader_dbOrderID" FOREIGN KEY (db_order_id) REFERENCES db.db_order_header(db_order_id) ON DELETE CASCADE;

ALTER TABLE ONLY db.db_order_detail
    ADD CONSTRAINT "FK_dbOrderDetail_SpecialOfferProduct_SpecialOfferIDProductID" FOREIGN KEY (special_offer_id, product_id) REFERENCES db.special_offer_product(special_offer_id, product_id);

ALTER TABLE ONLY db.db_order_header_db_reason
    ADD CONSTRAINT "FK_dbOrderHeaderdbReason_dbOrderHeader_dbOrderID" FOREIGN KEY (db_order_id) REFERENCES db.db_order_header(db_order_id) ON DELETE CASCADE;
...
ALTER TABLE ONLY db.db_order_header
    ADD CONSTRAINT "FK_dbOrderHeader_Address_BillToAddressID" FOREIGN KEY (bill_to_address_id) REFERENCES db.address(address_id);

ALTER TABLE ONLY db.db_order_header
    ADD CONSTRAINT "FK_dbOrderHeader_Address_ShipToAddressID" FOREIGN KEY (ship_to_address_id) REFERENCES db.address(address_id);

ALTER TABLE ONLY db.db_order_header
    ADD CONSTRAINT "FK_dbOrderHeader_CreditCard_CreditCardID" FOREIGN KEY (credit_card_id) REFERENCES db.credit_card(credit_card_id);

ALTER TABLE ONLY db.db_order_header
    ADD CONSTRAINT "FK_dbOrderHeader_CurrencyRate_CurrencyRateID" FOREIGN KEY (currency_rate_id) REFERENCES db.currency_rate(currency_rate_id);

ALTER TABLE ONLY db.db_order_header
    ADD CONSTRAINT "FK_dbOrderHeader_Customer_CustomerID" FOREIGN KEY (customer_id) REFERENCES db.customer(customer_id);

ALTER TABLE ONLY db.db_order_header
    ADD CONSTRAINT "FK_dbOrderHeader_dbdb_dbdbID" FOREIGN KEY (db_db_id) REFERENCES db.db_db(business_entity_id);

ALTER TABLE ONLY db.db_order_header
    ADD CONSTRAINT "FK_dbOrderHeader_dbTerritory_TerritoryID" FOREIGN KEY (territory_id) REFERENCES db.db_territory(territory_id);

ALTER TABLE ONLY db.db_order_header
    ADD CONSTRAINT "FK_dbOrderHeader_ShipMethod_ShipMethodID" FOREIGN KEY (ship_method_id) REFERENCES db.ship_method(ship_method_id);

ALTER TABLE ONLY db.db_db_quota_history
    ADD CONSTRAINT "FK_dbdbQuotaHistory_dbdb_BusinessEntityID" FOREIGN KEY (business_entity_id) REFERENCES db.db_db(business_entity_id);

ALTER TABLE ONLY db.db_db
    ADD CONSTRAINT "FK_dbdb_dbTerritory_TerritoryID" FOREIGN KEY (territory_id) REFERENCES db.db_territory(territory_id);
...
ALTER TABLE ONLY db.db_territory_history
    ADD CONSTRAINT "FK_dbTerritoryHistory_dbdb_BusinessEntityID" FOREIGN KEY (business_entity_id) REFERENCES db.db_db(business_entity_id);

ALTER TABLE ONLY db.db_territory_history
    ADD CONSTRAINT "FK_dbTerritoryHistory_dbTerritory_TerritoryID" FOREIGN KEY (territory_id) REFERENCES db.db_territory(territory_id);

ALTER TABLE ONLY db.db_territory
    ADD CONSTRAINT "FK_dbTerritory_CountryRegion_CountryRegionCode" FOREIGN KEY (country_region_code) REFERENCES db.country_region(country_region_code);
